...
public class ManifestDeclarator extends BaseDeclarator {
    ...
    protected void parseClassExpressions() {
        ...
        Iterators.flatMap(Iterators.flatMap(Iterators.of(RDFS.subClassOf, OWL.disjointWith), p -> listStatements(null, p, null)),
                        s -> Iterators.of(s.getSubject(), s.getObject()))
                .filterKeep(RDFNode::isResource).mapWith(RDFNode::asResource)
                .toSet()
                .forEach(this::declareClass);
        // "_:x owl:complementOf C"
        Iterators.flatMap(listStatements(null, OWL.complementOf, null), s -> Iterators.of(s.getSubject(), s.getObject()))
                .filterKeep(RDFNode::isResource).mapWith(RDFNode::asResource)
                .toSet()
                .forEach(this::declareClass);
        // "_:x rdf:type owl:AllDisjointClasses ; owl:members ( C1 ... Cn )"
        Iterators.flatMap(listStatements(null, RDF.type, OWL.AllDisjointClasses)
                        .filterKeep(s -> s.getSubject().isAnon()), s -> members(s.getSubject(), OWL.members))
                .toSet()
                .forEach(this::declareClass);
        // "CN owl:disjointUnionOf ( C1 ... Cn )"
        Iterators.flatMap(listStatements(null, OWL.disjointUnionOf, null)
                                .mapWith(Statement::getSubject).filterKeep(RDFNode::isURIResource),
                        x -> Iterators.concat(Iterators.of(x), members(x, OWL.disjointUnionOf)))
                .toSet()
                .forEach(this::declareClass);
    ...
    protected void parseDataRangeExpressions() {
        ...
        listStatements(null, OWL.onProperties, null)
                ...
                .mapWith(Statement::getSubject)
                .forEachRemaining(r -> {
                    Iterators.flatMap(Iterators.of(OWL.allValuesFrom, OWL.someValuesFrom), p -> listStatements(r, p, null))
    ...
    protected void parseOneOfExpression() {
        ...
        listStatements(null, OWL.oneOf, null)
                ...
                .forEachRemaining(s -> {
                    ...
                    if (Iterators.allMatch(Iterators.create(values), RDFNode::isLiteral)) {
    ...
    protected void parseObjectPropertyExpressions() {
        ...
        Iterators.flatMap(Iterators.of(OWL.InverseFunctionalProperty,
        ...
        // "P1 owl:inverseOf P2"
        Iterators.flatMap(listStatements(null, OWL.inverseOf, null).filterKeep(s -> s.getObject().isURIResource()),
                        s -> Iterators.of(s.getSubject(), s.getResource()))
                .toSet()
                .forEach(this::declareObjectProperty);
        // 	"P owl:propertyChainAxiom (P1 ... Pn)"
        Iterators.flatMap(listStatements(null, OWL.propertyChainAxiom, null), this::subjectAndObjects)
                .toSet()
                .forEach(this::declareObjectProperty);
    ...
    protected void parseIndividuals() {
        ...
        Iterators.flatMap(Iterators.flatMap(Iterators.of(OWL.sameAs, OWL.differentFrom), p -> listStatements(null, p, null)),
                        s -> Iterators.of(s.getSubject(), s.getObject()))
        ...
        Iterators.flatMap(listStatements(null, RDF.type, OWL.AllDifferent)
    ...
    private ExtendedIterator<Resource> disjointIndividuals(Resource s) {
        return Iterators.flatMap(Iterators.of(OWL.members, OWL.distinctMembers), p -> members(s, p));
    }

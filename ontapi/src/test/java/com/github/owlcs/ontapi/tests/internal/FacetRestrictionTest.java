...
public class FacetRestrictionTest extends ContentTestBase {
    ...
    public static List<Data> getData() {
        return TestDataCollection.getObjects().stream().filter(Data::isFacetRestriction).collect(Collectors.toList());
    }
    ...
    OWLObject fromModel(Data data) {
        ...
        OWLFacetRestriction ont = (OWLFacetRestriction) data.create(df);

        Ontology o = m.createOntology();
        o.add(df.getOWLDatatypeDefinitionAxiom(df.getOWLDatatype("D"),
                df.getOWLDatatypeRestriction(df.getStringOWLDatatype(), ont)));
        o.clearCache();
        OWLDatatypeRestriction restriction = (OWLDatatypeRestriction) o.axioms(AxiomType.DATATYPE_DEFINITION)

...
public class Builder {
    ...
    private final
    @Nonnull
    OWLAnnotationProperty ap = df.getOWLAnnotationProperty("urn:test#", "ann");
    private final
    @Nonnull
    ...
    private final
    @Nonnull
    OWLLiteral plainlit = df.getOWLLiteral("string", "en");
    private final
    @Nonnull
    IRI iri = IRI.create("urn:test#", "iri");
    private final
    @Nonnull
    Set<OWLAnnotation> as = Sets.newHashSet(df.getOWLAnnotation(ap, df.getOWLLiteral("test")));
    private final
    @Nonnull
    ...
    private final
    @Nonnull
    OWLNamedIndividual j = df.getOWLNamedIndividual("urn:test#", "j");
    private final
    @Nonnull
    OWLDatatype d = df.getOWLDatatype("urn:test#", "datatype");
    private final
    @Nonnull
    Set<OWLDataProperty> dps = Sets.newHashSet(df.getOWLDataProperty(iri), dp);
    private final
    @Nonnull
    Set<OWLObjectProperty> ops = Sets.newHashSet(df.getOWLObjectProperty(iri), op);
    private final
    @Nonnull
    Set<OWLClass> classes = Sets.newHashSet(df.getOWLClass(iri), ce);
    private final
    @Nonnull
    Set<OWLNamedIndividual> inds = Sets.newHashSet(i, df.getOWLNamedIndividual(iri));
    private final
    @Nonnull
    SWRLAtom v1 = df.getSWRLBuiltInAtom(IRI.create("urn:swrl#", "v1"), Arrays.asList(
            (SWRLDArgument) df.getSWRLVariable("urn:swrl#", "var3"), df.getSWRLVariable("urn:swrl#", "var4")));
    private final
    @Nonnull
    SWRLAtom v2 = df.getSWRLBuiltInAtom(IRI.create("urn:swrl#", "v2"), Arrays.asList(
    ...
    private final
    @Nonnull
    SWRLDArgument var1 = df.getSWRLVariable("urn:swrl#", "var1");
    private final
    @Nonnull
    List<SWRLDArgument> var1list = Arrays.asList(var1);
    private final
    @Nonnull
    SWRLIArgument var2 = df.getSWRLVariable("urn:swrl#", "var2");
    private final
    @Nonnull
    LinkedHashSet<SWRLAtom> body2 = Sets.newLinkedHashSet(Arrays.asList(v1, df.getSWRLClassAtom(

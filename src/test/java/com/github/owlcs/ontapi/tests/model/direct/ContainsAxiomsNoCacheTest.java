...
public class ContainsAxiomsNoCacheTest extends ContainsAxiomsTest {
    ...
    public <X extends OWLAxiom> void testAxiomTranslator(Class<X> type, ModelData data, Function<OWLDataFactory, X> get) {
        testAxiomTranslator(m -> ((Ontology) data.fetch(m)).asGraphModel(), type, get);
    }

    @ParameterizedTest(name = "[{index}] ::: type={0}")
    @MethodSource("dynamicData")
    public <X extends OWLAxiom> void testAxiomTranslator(Class<X> type,
                                                         Function<OntologyManager, OntModel> createModel,
                                                         Function<DataFactory, X> createAxiom) {
        testAxiomTranslator(m -> createModel.apply((OntologyManager) m), type, f -> createAxiom.apply((DataFactory) f));
        testAxiomTranslator(createModel, type);
    }

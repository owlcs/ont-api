...
public class ReadWriteUtils {
    ...
    private static File getFileToSave(String name, OntFormat type) {
        ...
        return new File(dir, name + "." + type.getExt());
    }
    ...
    public static void save(OWLOntology ontology, String name, OntFormat type) {
        File dst = getFileToSave(name, type);
        LOGGER.debug("Save owl-ontology to " + dst.toURI() + " (" + type.getType() + ")");
        try (FileOutputStream out = new FileOutputStream(dst)) {
            ontology.getOWLOntologyManager().saveOntology(ontology, type.getOwlFormat(), out);
        } catch (OWLOntologyStorageException | IOException e) {

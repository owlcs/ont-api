...
public class OntologyID extends OWLOntologyID {
    ...
    public OntologyID(String iri, String ver) {
        super(Optional.of(iri).map(IRI::create), Optional.ofNullable(ver).map(IRI::create));
    }
    ...
    /**
     * Answers the graph node that this ontology identifier represents.
     *
     * @return {@link Node}, not {@code null}
     * @throws IllegalStateException in case this instance was constructed incorrectly,
     *                               which can only happen if the OWL-API guys have changed its own implementation
     */
    public Node asNode() throws IllegalStateException {
        Optional<String> internalID = getField("internalID");
        if (internalID.isPresent()) {
            return NodeFactory.createBlankNode(BlankNodeId.create(internalID.get()));
        }
        Optional<IRI> iri = getField("ontologyIRI");
        return iri.map(IRI::getIRIString)
                .map(NodeFactory::createURI)
                .orElseThrow(() -> new IllegalStateException("No ontologyIRI or internalID"));
    }
    ...
    private <R> R getField(String name) {
        try {
            Field field = getClass().getSuperclass().getDeclaredField(name);
            field.setAccessible(true);

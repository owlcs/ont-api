...
public class DatatypeSearcher extends WithCardinality<OWLDatatype> implements ForDatatype {
    /**
     * All translators, since any axiom can be annotated and therefore contains a literal with datatype inside
     */
    ...
    private boolean isInAxiom(OntStatement statement, OntModel model, AxiomsSettings conf) {
        return containsAxiom(Iterators.create(getRootStatements(model, statement)), conf);
    }
}

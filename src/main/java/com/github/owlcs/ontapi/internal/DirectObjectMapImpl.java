...
public class DirectObjectMapImpl<X extends OWLObject> implements ObjectMap<X> {
    ...
    public DirectObjectMapImpl(Supplier<Iterator<ONTObject<X>>> loader) {
        this(Objects.requireNonNull(loader),
                k -> Iterators.findFirst(Iterators.create(loader.get()).filterKeep(x -> x.getOWLObject().equals(k))));
    }
    ...
    public DirectObjectMapImpl(Supplier<Iterator<ONTObject<X>>> loader,
        ...
        this(loader, finder, x -> finder.apply(x).isPresent());
    }
    ...
    public ExtendedIterator<ONTObject<X>> listONTObjects() {
        return Iterators.create(list.get());
    }
    ...
    public Stream<X> keys() {
        return Iterators.asStream(listONTObjects().mapWith(ONTObject::getOWLObject));
    }

    @Override
    public Stream<ONTObject<X>> values() {
        return Iterators.asStream(listONTObjects());
    }

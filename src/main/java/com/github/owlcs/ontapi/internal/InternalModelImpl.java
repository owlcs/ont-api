 ...
 */
public class InternalModelImpl extends OntGraphModelImpl implements InternalModel, HasObjectFactory, HasConfig {
    private static final Logger LOGGER = LoggerFactory.getLogger(InternalModelImpl.class);
     ...
     */
    private final InternalCache.Loading<InternalModelImpl, InternalConfig> config;
    /**
     ...
     */
    protected final InternalCache.Loading<InternalModelImpl, ModelObjectFactory> objectFactory;
    /**
     ...
     * @see SearchModel
     */
    protected final InternalCache.Loading<InternalModelImpl, OntGraphModelImpl> searchModel;
    /**
     ...
     * @see ObjectMap
     */
    protected final InternalCache.Loading<InternalModelImpl, Map<OWLTopObjectType, ObjectMap<? extends OWLObject>>> content;
    /**
     ...
     * @see ObjectMap
     */
    protected final InternalCache.Loading<InternalModelImpl, Map<OWLComponentType, ObjectMap<OWLObject>>> components;
    /**
     ...
     * through the main ({@link InternalModelImpl this}) interface.
     *
     ...
     * otherwise this same {@link InternalModelImpl} instance with no optimizations will be returned.
    ...
    protected OntGraphModelImpl createSearchModel() {
        ...
        return new SearchModel(getGraph(), getOntPersonality(), getConfig()) {
            ...
            @Nonnull
            public ModelObjectFactory getObjectFactory() {
                return InternalModelImpl.this.getObjectFactory();
            }
        };
    }
    ...
    public Stream<OWLDeclarationAxiom> listOWLDeclarationAxioms(OWLEntity entity) {
        ...
        if (!useAxiomsSearchOptimization(config)) {
            return InternalModel.super.listOWLDeclarationAxioms(entity);
        }
    ...
    public Stream<OWLAnnotationAssertionAxiom> listOWLAnnotationAssertionAxioms(OWLAnnotationSubject subject) {
        ...
        if (!useAxiomsSearchOptimization(config)) {
            return InternalModel.super.listOWLAnnotationAssertionAxioms(subject);
        }
    ...
    public Stream<OWLDataPropertyAssertionAxiom> listOWLDataPropertyAssertionAxioms(OWLIndividual subject) {
        InternalConfig config = getConfig();
        if (!useAxiomsSearchOptimization(config)) {
            return InternalModel.super.listOWLDataPropertyAssertionAxioms(subject);
        }
    ...
    public Stream<OWLObjectPropertyAssertionAxiom> listOWLObjectPropertyAssertionAxioms(OWLIndividual subject) {
        InternalConfig config = getConfig();
        if (!useAxiomsSearchOptimization(config)) {
            return InternalModel.super.listOWLObjectPropertyAssertionAxioms(subject);
        }
    ...
    public Stream<OWLObjectPropertyRangeAxiom> listOWLObjectPropertyRangeAxioms(OWLObjectPropertyExpression subject) {
        ...
        if (!ObjectPropertyRangeBySubject.isSupported(subject) || !useAxiomsSearchOptimization(config)) {
            return InternalModel.super.listOWLObjectPropertyRangeAxioms(subject);
        }
    ...
    public Stream<OWLObjectPropertyDomainAxiom> listOWLObjectPropertyDomainAxioms(OWLObjectPropertyExpression subject) {
        InternalConfig config = getConfig();
        if (!ObjectPropertyRangeBySubject.isSupported(subject) || !useAxiomsSearchOptimization(config)) {
            return InternalModel.super.listOWLObjectPropertyDomainAxioms(subject);
        }
    ...
    public Stream<OWLClassAssertionAxiom> listOWLClassAssertionAxioms(OWLIndividual subject) {
        InternalConfig config = getConfig();
        if (!useAxiomsSearchOptimization(config)) {
            return InternalModel.super.listOWLClassAssertionAxioms(subject);
        }
    ...
    public Stream<OWLClassAssertionAxiom> listOWLClassAssertionAxioms(OWLClassExpression object) {
        ...
        if (!ClassAssertionByObject.isSupported(object) || !useAxiomsSearchOptimization(config)) {
            return InternalModel.super.listOWLClassAssertionAxioms(object);
        }
    ...
    public Stream<OWLSubClassOfAxiom> listOWLSubClassOfAxiomsBySubject(OWLClass subject) {
        InternalConfig config = getConfig();
        if (!useAxiomsSearchOptimization(config)) {
            return InternalModel.super.listOWLSubClassOfAxiomsBySubject(subject);
        }
    ...
    public Stream<OWLSubClassOfAxiom> listOWLSubClassOfAxiomsByObject(OWLClass object) {
        InternalConfig config = getConfig();
        if (!useAxiomsSearchOptimization(config)) {
            return InternalModel.super.listOWLSubClassOfAxiomsByObject(object);
        }
    ...
    protected ObjectMap<OWLObject> createComponentObjectMap(OWLComponentType type) {
        InternalConfig conf = getConfig();
        Supplier<Iterator<ONTObject<OWLObject>>> loader = () -> InternalModelImpl.this.listOWLObjects(type, conf);

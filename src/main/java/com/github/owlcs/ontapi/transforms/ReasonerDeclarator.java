 ...
 * </ul>
 * <p>
 * Note: ObjectProperty &amp; ClassExpression have more priority than DataProperty &amp; DataRange
 */
...
public class ReasonerDeclarator extends BaseDeclarator {
    ...
    protected void parseDataAndObjectRestrictions() {
        ...
        Iterators.flatMap(Iterators.of(OWL.allValuesFrom, OWL.someValuesFrom),
    ...
    protected boolean mustBeDataOrObjectProperty(Resource candidate) {
        ...
        return Iterators.anyMatch(Iterators.flatMap(lists, RDFList::iterator), candidate::equals);
    }
    ...
    protected boolean canBeDataPropertyInAssertion(Property candidate) {
        ...
        return Iterators.findFirst(listStatements(null, candidate, null)
    ...
    protected Res testEquivalentClasses(Statement statement) {
        ...
        if (Iterators.anyMatch(Iterators.of(a, b), this::isClassExpression)) {
            declareClass(a);
            declareClass(b);
            return Res.TRUE;
        }
        if (a.isURIResource() && Iterators.anyMatch(Iterators.of(a, b), this::isDataRange)) {
            declareDatatype(a);
            declareDatatype(b);
            return Res.TRUE;
        }
    ...
    protected void parseUnionAndIntersectionClassExpressions() {
        ...
        Iterators.flatMap(Iterators.of(OWL.unionOf, OWL.intersectionOf), p -> listStatements(null, p, null))
    ...
    protected Res testUnionAndIntersectionClassExpressions(Statement statement) {
        ...
        if (Iterators.anyMatch(Iterators.create(set), this::isClassExpression)) {
            set.forEach(this::declareClass);
            return Res.TRUE;
        }
        if (Iterators.anyMatch(Iterators.create(set), this::isDataRange)) {
            set.forEach(this::declareDatatype);
            return Res.TRUE;
        }
        if (decider.chooseClass()) {
            if (Iterators.allMatch(Iterators.create(set), this::canBeClass)) {
                set.forEach(this::declareClass);
            } else if (Iterators.allMatch(Iterators.create(set), this::canBeDatatype)) {
                set.forEach(this::declareDatatype);
            } else {
                return Res.FALSE;
            }
    ...
    protected void parseEquivalentAndDisjointProperties() {
        Iterators.flatMap(Iterators.of(OWL.equivalentProperty, OWL.propertyDisjointWith), p -> listStatements(null, p, null))
                .filterKeep(s -> s.getObject().isResource())
